...
class SigninEffectHandler(
  ...
  override fun accept(value: SigninEffect) {
    launch {
      when (value) {
        is SigninEffect.SignIn -> handleSignIn(value)
      }
    }
  }
  ...
  private suspend fun handleSignIn(effect: SigninEffect.SignIn) {
    val (email, password) = effect
    try {
      val response = conduitService.login(email, password)
